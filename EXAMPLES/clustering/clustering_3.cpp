/***************************************************************************
                             EXAMPLES/clustering/clustering_3.cpp  description
...
void clustering_3_example() {
    ...
    cout<<"  - strategy :"<<endl;
    cout<<"    - nbTry = 5 "<<endl;
    cout<<"    - 2 chained algorithms"<<endl;
    cout<<"      - SEM (80 iterations)"<<endl;
    cout<<"      - EM (epsilon=0.0001)"<<endl;
    cout<<"-----------------------------------------------------------------------"<<endl<<endl;
    ...
    // Strategy example :
    // - nbTry = 5
    // - algo 1 : SEM (nbIteration = 80)
    // - algo 2 : EM (epsilon=0.0001)
    // Note : default algorithm is EM.
    //------------------------------------
    cInput->getStrategy()->setNbTry ( 5 );

    // SEM
    cInput->getStrategy()->insertAlgo ( XEM::SEM, 0 ); // SEM is first and EM is now 2nd
    cInput->getStrategy()->setAlgoIteration ( 0, 80 ); // nbIteration=80 for the 1rst algorithm (SEM)

    // EM
    // default stop rule is NBITERATION_EPSILON
    cInput->getStrategy()->setAlgoStopRule ( XEM::EPSILON, 1 ); // change the stop rule for the 2nd (position=1) algorithm (EM)
    cInput->getStrategy()->setAlgoEpsilon ( 1, 0.0001 ); // epsilon=0.0001 for the 2nd algorithm (EM)

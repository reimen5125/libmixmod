/***************************************************************************
                             SRC/mixmod/Kernel/Parameter/BinaryEjParameter.h  description
...
@author F LANGROGNET
 */
class BinaryEjParameter : public BinaryParameter {
...
public:
	...
	/// Constructor
	// called by XEMModel
	BinaryEjParameter(Model * iModel, ModelType * iModelType, int * tabNbModality);

	/// Constructor copy
	BinaryEjParameter(const BinaryEjParameter * iParameter);

	/// Destructor
	~BinaryEjParameter();

	/// Comparison operator
	virtual bool operator ==(const BinaryEjParameter & param) const;
...
inline float * BinaryEjParameter::getScatter() const {
	return _scatter;
}
